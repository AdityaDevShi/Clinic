...
function BookingContent() {
    ...
    useEffect(() => {
        const fetchSlots = async () => {
            ...
            try {
                const slots = await BookingService.getAvailableSlots(therapistId, selectedDate);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-[var(--neutral-50)]">
                <div className="text-center p-8 bg-white rounded-2xl shadow-sm max-w-md mx-4">
                    ...
                    <h2 className="text-xl font-serif text-[var(--primary-800)] mb-2">Therapist Access Restricted</h2>
                    <p className="text-[var(--neutral-600)] mb-6">
                        Therapists cannot book sessions for themselves or others. Please sign in with a client account to book an appointment.
                    </p>
                    <Link href="/therapist/dashboard" className="inline-flex items-center justify-center px-6 py-2 bg-[var(--primary-600)] text-white rounded-lg hover:bg-[var(--primary-700)] transition-colors">
                        Return to Dashboard
                    </Link>
                </div>
            </div>
